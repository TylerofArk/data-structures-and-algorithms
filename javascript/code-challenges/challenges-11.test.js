...
// Collabed with Luis Rosales & Dan Awesome
...
const count = (target, input) => {
  return input.flat().filter(value => value === target).length;
};
...
const totalSum = (input) => {
  return input.flat().reduce((prenum, currentnum) => prenum + currentnum, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let numbersOnly = input.map((v) => {
    ...
    return v.filter((val, i) => {
      if (typeof(val) !== 'string' && val % 5 === 0){
        return val;
      }
    });
  });
